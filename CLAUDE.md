...
- **Content-type builder (done):** the no-code field designer (the flexibility selling point). `apps/core/src/content/content-types.controller.ts` — GET (any member) + POST/PATCH/DELETE (OWNER/ADMIN), delete blocked if entries exist, field-key uniqueness, audit-logged, under RLS. `ContentType.schema` JSON = `{ fields: FieldDef[] }` (types: text/textarea/richtext/number/date/boolean/select/image). `apps/admin` ContentTypes screen at `/content/types` — type list + create + per-field editor (key/label/type/required/options). **Follow-up:** the entry Editor still renders fixed title/summary/body — wiring it to render a type's defined fields is the next step.

Deferred: editor renders dynamic schema fields, media library, translation UI (admin locale tabs display-only; only EN content published), prod app-role split, Docker image build-test, and the **ops/governance track** (real pilot ministry, deploy target/sovereign infra, RISA TYPO3-clause). Open decisions still: pilot ministry + project name.
