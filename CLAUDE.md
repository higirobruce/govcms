...
- **Media library (done):** `apps/core/src/media` — upload (multipart, 10MB limit), list, per-locale alt text, delete; files on local disk under `apps/core/uploads/<tenantId>/` (gitignored), served statically at `/uploads/` (prod swaps for object storage + CDN; `Media.path` abstracts location). RLS-scoped via `tenantTx`, audit-logged. `apps/admin` Media screen at `/content/media` — upload, grid with thumbnails, alt-status badges (alt ✓ / alt missing), alt editing, delete. **Follow-up:** the public site doesn't yet render entry images (media is admin-side so far).
